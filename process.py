...
def wrist_detect(img):
    ...
    for i, img_roi in enumerate([cun_roi, guan_roi, chi_roi]):
        ...
        im2,contours, _ = cv2.findContours(global_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)##im2变量是为了适配py3.6对应的opencv有三个变量，新的是2个
        #contours, _ = cv2.findContours(global_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  #py3.9
        ROI = img_roi
        if contours:
